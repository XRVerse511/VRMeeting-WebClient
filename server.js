    ...
    try {
        ...
        if (request.method !== "GET" || uri.indexOf("..") !== -1) {
            try {
                ...
                response.write(
                    "401 Unauthorized: " + path.join("/", uri) + "\n"
                );
                response.end();
                return;
            } catch (e) {
        ...
        try {
            if (fs.statSync(filename).isDirectory()) {
                ...
                if (filename.indexOf(resolveURL("/demos/MultiRTC/")) !== -1) {
                    ...
                    filename.indexOf(resolveURL("/demos/dashboard/")) !== -1
                ) {
                    filename = filename.replace(
                        resolveURL("/demos/dashboard/"),
                        ""
                    );
                    filename += resolveURL("/demos/dashboard/index.html");
                } else if (
                    filename.indexOf(resolveURL("/demos/video-conference/")) !==
                    -1
                ) {
                    filename = filename.replace(
                    ...
                    filename += resolveURL(
                        "/demos/video-conference/index.html"
                    );
