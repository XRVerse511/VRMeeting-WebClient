...
export function HandData() {
    ...
    const joints = {
        ...
        I_Proximal: new JointObject(
            "index-finger-phalanx-proximal",
            num++,
            this
        ),
        I_Intermediate: new JointObject(
        ...
        M_Proximal: new JointObject(
            "middle-finger-phalanx-proximal",
            num++,
            this
        ),
        M_Intermediate: new JointObject(
        ...
        R_Proximal: new JointObject(
            "ring-finger-phalanx-proximal",
            num++,
            this
        ),
        R_Intermediate: new JointObject(
        ...
        L_Proximal: new JointObject(
            "pinky-finger-phalanx-proximal",
            num++,
            this
        ),
        L_Intermediate: new JointObject(
    ...
    this.updateData = (controller, frame, referenceSpace) => {
        ...
        validPoses = frame.fillPoses(
            controller.hand.values(),
            referenceSpace,
            transforms
        );
    ...
    // "normal" helper
    const normalQuaternion = new THREE.Quaternion().setFromEuler(
    ...
    this.getDirection = (id, _vector) => {
        ...
        return vector
            .fromArray(transforms, mtxOffset + 8)
            .normalize()
            .negate();
    };
    ...
    this.getQuaternion = (id, _quaternion) => {
        const mtxOffset = id * 16;
        let quaternion = tmpQuaternion.clone();
        var idx = 0;
        for (var i = mtxOffset; i <= mtxOffset + 15; i++) {
    ...
    this.getPosition = (id, _vector) => {
        ...
        const mtxOffset = id * 16;
        const wristPos = this.getWristPosition();

        // get relative position
        let vector = tmpVector.clone();
        vector.fromArray(transforms, mtxOffset + 12);
        return vector;
    };
    this.getRadius = (id) => {
        return radii[id];
    ...
    this.getWristPosition = () => {
        let vector = tmpVector.clone();
        vector.fromArray(transforms, 12);
        return vector;
    };
    this.getWristQuaternion = () => {
        let quaternion = tmpQuaternion.clone();
        let idx = 0;
        for (let i = 0; i <= 15; i++) {
            rotMtx.elements[idx++] = transforms[i];
