...
export default class Signaling extends EventTarget {
    ...
    url(method) {
        return (
            Config.getWebRTCServer(location.protocol) + "/signaling/" + method
        );
    }
    ...
    async loopGetAnswer() {
        ...
        while (true) {
            ...
            answers.forEach((answer) => {
                this.dispatchEvent(
                    new CustomEvent("answer", { detail: answer })
                );
            });
        ...
        while (true) {
            ...
            if (candidates.length > 0) {
                for (let candidate of candidates[0].candidates) {
                    this.dispatchEvent(
                        new CustomEvent("candidate", { detail: candidate })
                    );
                }
            }
...
export class WebSocketSignaling extends EventTarget {
    constructor() {
        ...
        if (location.protocol === "https:") {
            var websocketUrl = Config.getWebRTCServer("wss:");
        } else {
            var websocketUrl = Config.getWebRTCServer("ws:");
        }
