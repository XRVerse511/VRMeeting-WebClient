...
function storePreviousState(gamepad) {
    ...
    gamepad.buttons.forEach(function (button, index) {
        gamepadsPreviousButtonsStates[gamepad.index][index] = {
            value: button.value,
...
function checkAxes(gamepad, previousGamePad) {
    for (var i = 0; i < gamepad.axes.length; i += 2) {
        ...
        var event = null;
        if (absX > _e || absY > _e) {
            event = new GamepadAxisEvent("gamepadAxis", {
                id: gamepadsConnectedTimeStamp[gamepad.index],
                index: i / 2 + _axisOffset,
                x: gamepad.axes[i] * _axisMultiplier,
                y: gamepad.axes[i + 1] * _axisMultiplier * _axisYInverted
            });
            ...
            if (previousAbsX > _e || previousAbsY > _e) {
                event = new GamepadAxisEvent("gamepadAxis", {
                    id: gamepadsConnectedTimeStamp[gamepad.index],
                    index: i / 2 + _axisOffset,
                    x: 0.0,
...
function gameLoop() {
    Object.keys(gamepadsPreviousAxesStates).forEach(function (gamepadIndex) {
        var gamepad = navigator.webkitGetGamepads
            ? navigator.webkitGetGamepads()[gamepadIndex]
            : navigator.getGamepads()[gamepadIndex];
        var previousButtons = gamepadsPreviousButtonsStates[gamepadIndex];
        gamepad.buttons.forEach(function (button, index) {
            var buttonStatus = navigator.webkitGetGamepads
                ? button == 1
                : button.value > 0 || button.pressed == true;
            var previousButtonStatus = navigator.webkitGetGamepads
                ? previousButtons[index].value == 1
                : previousButtons[index].value > 0 ||
                  previousButtons[index].pressed == true;
            if (buttonStatus != previousButtonStatus) {
                var event;
                if (buttonStatus)
                    event = new GamepadButtonEvent("gamepadButtonDown", {
                        id: gamepadsConnectedTimeStamp[gamepad.index],
                        index: index,
                        value: button.value
                    });
                else
                    event = new GamepadButtonEvent("gamepadButtonUp", {
                        id: gamepadsConnectedTimeStamp[gamepad.index],
                        index: index,
                        value: 0
                    });
                ...
                var event = new GamepadButtonEvent("gamepadButtonPressed", {
                    id: gamepadsConnectedTimeStamp[gamepad.index],
                    index: index,
                    value: button.value
                });
